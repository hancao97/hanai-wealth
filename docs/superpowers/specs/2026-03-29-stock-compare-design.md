...
### 2.1 标识符约定（避免与 `symbol`/展示代码混淆）

- Query 参数 **`compareStockId`** 的值**必须**与当日 JSON 每条记录上的 **`stockid`** 字段一致（与现有详情页 `route.query.stockid`、`allStocksData.find(item => item.stockid === …)`、以及 GuruFocus 请求路径中的 `{stockid}` **同源**）。
- 选券 UI 可对 `company`、`symbol` 等做展示与过滤，但**写入 URL 与状态机的唯一键**始终是 **`stockid`**。
...
- Query：`compareStockId=<stockid>`，语义与上节 **2.1** 一致。非法或不在当日列表：清除对比相关 query，可选轻提示。
...
- 交集过短或为空：图表区「数据不足」空态；**指标表仍展示**（来自当日 JSON）。**最小可展示点数或最短时间跨度**在 implementation plan 中定义一条验收阈值（避免「能画一点但无意义」与「完全空」边界不清）。
...
- **主标的**在对比图侧请求失败：在 **writing-plans 阶段选定唯一策略**（例如整图降级说明，或仅展示对比侧 + 说明），并写入验收口径；不得导致未捕获异常。
- **清空对比**：清除 `compareStockId` 并移除 query。

### 4.5 边界行为（writing-plans 中选定唯一策略并验收）

- 选中对比标的与主标的相同：禁止或自动清空（二选一，与测试用例一致）。
