...
| **Create** `src/composables/useCompareValuation.js` | `fetchValuation(stockid)` 封装 GuruFocus URL；`loadCompare(primaryStockid, compareStockid)` 使用 `Promise.allSettled`；调用 `mergeCompareValuationSeries`；暴露 `loading`、`error`、`merged`、`primarySettled`、`compareSettled`（便于父组件写降级文案）。 |
| **Create** `src/components/CompareValuationChart.vue` | Props：`primaryStock`、`compareStock`（行对象，含 `stockid`/`company`）、`primaryLabel`/`compareLabel` 可选。内部 `useCompareValuation` + `echarts` 四线（名称含股票简称或 `exchange_:symbol` 防混淆）；resize/dispose 与 `ValuationChart` 同模式；主请求失败 → 整图 `chart-error` 类 UI。 |
| **Create** `src/components/StockComparePanel.vue` | 折叠入口、「清除对比」、搜索输入、下拉/列表（过滤 `allStocksData`，`stockid !== primaryStockid`）；快照表至少：`company`、`exchange_`+`symbol`、`pettm`、`pb`、`mktcap_norm_currency`（字段名与 JSON 一致，格式复用列表页 `formatPE`/`formatMarketCap` 逻辑可抽或局部复制）；挂载 `CompareValuationChart`；样式类名加前缀 **`stock-compare-`**，避免与全局冲突。 |
...
创建 `src/utils/compareValuationMerge.test.js`，用**合成数据**：两段 `medps`/`price` 时间序列，重叠日期 ≥5 点时期望 `ok: true` 且四条 series 长度一致；重叠 <5 点时期望 `ok: false` 且 `code === 'INSUFFICIENT_OVERLAP'`。先 `import { mergeCompareValuationSeries } from './compareValuationMerge.js'`，实现体可暂时 `throw new Error('todo')`。
...
实现 `mergeCompareValuationSeries`：依赖下一 Task 抽出的 `alignValueDataToPrice`（可先 **复制粘贴** `ValuationChart` 中函数到 `valuationAlign.js` 并在本文件中 import，Task 2 再改 ValuationChart 引用）。内连接规则：对每只标的先得到 `alignedMedps` 与 `priceData` 等长索引，再按 `new Date(ts).toDateString()` 建 Map，取两 Map 的日期键交集，排序后生成四个数组 `[ts, y]`。少于 5 个交点则返回 `INSUFFICIENT_OVERLAP`。
...
### Task 2: 抽出 valuationAlign 并接通 ValuationChart

**Files:**
- Create: `src/utils/valuationAlign.js`
- Modify: `src/components/ValuationChart.vue`（删除重复函数，增加 import）

- [ ] **Step 1: 剪切** `alignValueDataToPrice`、`interpolateValue` **及**其仅被二者使用的辅助逻辑到 `valuationAlign.js`，并 **export**。

- [ ] **Step 2:** 在 `ValuationChart.vue` 中 `import { alignValueDataToPrice } from '@/utils/valuationAlign.js'`（路径以项目别名为准；若无 `@` 则用相对路径 `../utils/valuationAlign.js`）。
...
- [ ] **Step 2:** `watch` `primaryStock.stockid`、`compareStock.stockid`，变化时调用 `loadCompare`。
...
- [ ] **Step 5:** 嵌入 `CompareValuationChart`，传入两只行数据。
...
- [ ] **Step 4:** URL 带合法 `compareStockId` 时 `expanded` 默认 true（可用 `ref(true)` + 根据是否有 compare 初始化）。

- [ ] **Step 5:** 模板插入位置建议：`ValuationChart` 下方、`FinancialMetrics` 上方。
...
- [ ] **Step 2:** 故意错误 `compareStockId`，确认被清除。
